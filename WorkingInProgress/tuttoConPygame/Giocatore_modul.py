...
class Giocatore():
	def __init__(self, game, caselle, percorso, tag):
		self.game = game
	...
	def creaCasellaIniziale(self):
		...
		casellaIniziale = Casella(self.display, x, y, self.tag)
		
		self.draw_text(self.tag, 12, casellaIniziale.getCenterX(), casellaIniziale.getCenterY())
		

	def draw_text(self, text, size, x, y):
		font = pygame.font.Font(self.game.font_name,size)
		text_surface = font.render(text, True, (255,0,0,1))
	...
	def posiziona(self, spostamento):
		...
		try:
			#TODO Elimina la casella iniziale (l'ellisse prima dell'inizio del percorso)
			pass
		except Exception:
			pass
